...
public class ListSorts {
  ...
  public static LinkedQueue makeQueueOfQueues(LinkedQueue q) {
    ...
    while (!q.isEmpty()) {
      try {
        LinkedQueue queueElement = new LinkedQueue();
        queueElement.enqueue(q.dequeue());
        newQueue.enqueue(queueElement);
  ...
  public static LinkedQueue mergeSortedQueues(LinkedQueue q1, LinkedQueue q2) {
    ...
    try {
      while (!q1.isEmpty() && !q2.isEmpty()) {
        compare = ((Comparable) q1.front()).compareTo((Comparable) q2.front());
        if (compare < 0) {
  ...
  public static void quickSort(LinkedQueue q) {
    // Your solution here.
    int randomIndex = (int) (Math.random() * q.size() + 1);
