...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    ...
    int compare = key.compareTo((Comparable)node.entry.key());
    if(compare == 0) {
      return node;
  ...
  public Entry remove(Object key) {
    // Replace the following line with your solution.
    BinaryTreeNode node = findHelper((Comparable) key, this.root);
    if(node == null) {
      return null;
    }
    ...
    if(node.leftChild == null && node.rightChild == null) {
      ...
      BinaryTreeNode subtreeNode = node.rightChild;
      while(subtreeNode.leftChild != null) {
        subtreeNode = subtreeNode.leftChild;
      }
      Entry newEntry = this.remove(subtreeNode.entry.key());
      node.entry = newEntry;
      size++;
    }
    size--;
