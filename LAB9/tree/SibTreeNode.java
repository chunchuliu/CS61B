...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(isValidNode()) {
      ...
      SibTreeNode children = this.firstChild;
      while(c > 2 && children.nextSibling().isValidNode()) { 
        c--;
        System.out.println("Now is the node with item: " + children.item());
        children = (SibTreeNode) children.nextSibling();
       
      }

        if(!children.nextSibling().isValidNode()) {
          children.nextSibling = newNode;
        } else {
          newNode.nextSibling = children.nextSibling;
          children.nextSibling = newNode;
        }
      ...
      if(this.children() == 0) {
        ...
        if(this.parent.children() == 1) {
          ...
          if(!this.nextSibling().isValidNode()) {
            SibTreeNode kid = this.parent.firstChild;
            ...
            this.parent.firstChild = (SibTreeNode) this.nextSibling();
            this.valid = false;
