...
public class DList extends List {
  ...
  public void insertFront(Object item) {
    ...
    //   the `list' field (second parameter) as well.
    if(size == 0){
      DListNode node = new DListNode(item, this, this.head, this.head);
      this.head.next = node;
      this.head.prev = node;
    } else {
      DListNode node = new DListNode(item, this, this.head, this.head.next);
      node.next.prev = node;
      this.head.next = node;
    }
    size++;
  ...
  public void insertBack(Object item) {
    ...
    //   the `list' field (second parameter) as well.
    if(size == 0) {
      DListNode node = new DListNode(item, this, head, head);
      head.next = node;
      head.prev = node;
    } else {
      DListNode node = new DListNode(item, this, head.prev, head);
      head.prev.next = node;
      head.prev = node;  
    }
    size++;
